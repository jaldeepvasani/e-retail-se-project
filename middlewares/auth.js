...
const sellerAdminAuth = (req, res, next) => {
  const { isSeller,isAdmin } = req.user;

  if (!isSeller && !isAdmin) {
    res.status(401).json({ message: "You aren't a admin or seller" });
  } else next();
};
...
module.exports = { auth, sellerAuth, adminAuth, shipperAuth ,sellerAdminAuth};
